...
void vf_filter_frame(struct vf_instance *vf, struct mp_image *img)
{
    ...
    if (vf->filter_n) {
        struct mp_image **res = vf->filter_n(vf, img);
        for (int n = 0; res[n]; n++)
            MP_TARRAY_APPEND(vf, vf->out_queued, vf->num_out_queued, res[n]);
        talloc_free(res);
    } else {
        struct mp_image *res = vf->filter(vf, img);
        if (res)
            MP_TARRAY_APPEND(vf, vf->out_queued, vf->num_out_queued, res);
    }
}
