...
typedef struct vf_instance {
    ...
    // Like filter(), but return multiple output images. The array is
    // terminated with NULL, and will be free'd with talloc_free() by the
    // caller.
    struct mp_image **(*filter_n)(struct vf_instance *vf, struct mp_image *mpi);
...
void vf_filter_frame(struct vf_instance *vf, struct mp_image *img);
