...
/* Memory management:
 * - mp_image is a reference to the actual image data (pixels). The actual
 *   image data is reference counted. mp_image allocations are light-weight
 *   references, and can be created with mp_image_new_ref() and free'd with
 *   talloc_free() (the helpers mp_image_setrefp() and mp_image_unrefp() can
 *   also be used). The actual image data is free'd when the last reference
 *   to it is free'd.
 * - Each mp_image has a clear owner. The owner can do anything with it, such
 *   as changing mp_image fields. Instead of making ownership ambiguous, new
